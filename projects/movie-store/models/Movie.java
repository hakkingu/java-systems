...
public class Movie {
    ...
    public Movie(String name, String format, double rating) {
        if (name == null || name.isBlank()) {
            throw new IllegalArgumentException("The name must contain some kind of value");
        }
        if (!(format.equals("DVD") || format.equals("Blue-Ray"))) {
            throw new IllegalArgumentException("The format must be DVD, or BLUE-RAY");
        }
        if (rating < 0 || rating > 10) {
            throw new IllegalArgumentException("That's a invalid rating");
        }
        this.name = name;
        this.format = format;
        this.rating = rating;
